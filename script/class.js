    ...
    constructor(theme, id = null, title = null, themeColor = "#DDDDDD") {
        ...
        if (this.id == null) {
            ...
            fetch('script_php/get_tasks.php', {
                method: 'POST', body: JSON.stringify({theme_id: this.id})
            }).then((response) => {
                const contentType = response.headers.get("content-type");
                if (contentType && contentType.indexOf("application/json") !== -1) {
                    return response.json().then((json) => {
        ...
        if (this.EDITING) {
            ...
            if (input.value.trim()) {
                ...
                fetch('script_php/edit_theme.php', {
                    method: 'POST', body: JSON.stringify({
                        theme_id: this.id, new_title: newTitle, new_color: this.editColorButton.value
                    })
                }).then((response) => {
                    const contentType = response.headers.get("content-type");
                    if (contentType && contentType.indexOf("application/json") !== -1) {
            ...
            if (input.value.trim()) {
                ...
                fetch('script_php/add_people.php', {
                    method: 'POST', body: JSON.stringify({
                        theme_id: this.id, username: username
                    })
                }).then((response) => {
                    const contentType = response.headers.get("content-type");
                    if (contentType && contentType.indexOf("application/json") !== -1) {
        ...
        if (bool && title != null && id != null) {
            ...
            if (this.contentNewTaskInput.value.trim()) {
                fetch('script_php/create_task.php', {
                    method: 'POST', body: JSON.stringify({
                        theme_id: this.id, task_name: this.contentNewTaskInput.value
                    })
                }).then((response) => {
                    const contentType = response.headers.get("content-type");
                    if (contentType && contentType.indexOf("application/json") !== -1) {
...
class todoTask {
    ...
    validateClick() {
        if (this.EDITING) {
            ...
            fetch('script_php/edit_task.php', {
                method: 'POST', body: JSON.stringify({
                    task_id: this.id, new_title: newTitle
                })
            }).then((response) => {
                const contentType = response.headers.get("content-type");
                if (contentType && contentType.indexOf("application/json") !== -1) {
    ...
    deleteClick(keepInDatabase = false) {
        ...
        fetch('script_php/delete_task.php', {
            method: 'POST', body: JSON.stringify({task_id: this.id})
        })
            .then((response) => {
                const contentType = response.headers.get("content-type");
                if (contentType && contentType.indexOf("application/json") !== -1) {
    ...
    checkboxClick(changeDatabase = true) {
        ...
        if (this.checkbox.checked) {
            ...
            fetch('script_php/task_state.php', {
                method: 'POST', body: JSON.stringify({
                    task_id: this.id, task_status: this.status
                })
            })
                .then((response) => {
                    const contentType = response.headers.get("content-type");
